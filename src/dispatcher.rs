...
impl Dispatcher {
    pub fn new() -> (Self, UnboundedSender<Action>) {
        ...
        (Dispatcher { action_rx, stores_tx: Vec::new() }, action_tx)
    }
    ...
    pub async fn dispatch(mut self) {
        loop {
            ...
            let action = self.action_rx.recv().await.expect("Channel has been closed");
            // println!("action founded: {:?}", action);
            for store_rx in &self.stores_tx {
                store_rx.send(action).expect("Error while sending the action");
            }
        }
    }
}
